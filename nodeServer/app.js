      ...
      if (next.value instanceof Promise) {
        ...
        if (next.value instanceof Object) {
          ...
          if (next.value.do === "AnnounceRole") {
            // 역할 공지라면
            emitter
              .to(member.find(o => o.name == next.value.name).socket)
              .emit("ROLE_ALERT", `${next.value.role}`);
            ...
            for (let name of next.value.nameList) {
              emitter
                .to(member.find(o => o.name == name).socket)
                .emit("RESULT_OF_INVESTIGATION", {
            ...
            for (let tempMember of next.value.nameList) {
              ...
              emitter
                .to(tempSocket.socket)
                .emit("UPDATE_LIST", next.value.nameList);
            }
